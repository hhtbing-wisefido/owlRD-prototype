...
所有5个管理页面已完整实现CRUD功能！现在可以对所有数据进行创建、查看、编辑和删除操作。
...
### 3. Locations (位置管理) ✅
**文件**: `frontend/src/pages/Locations.tsx`
- ✅ 创建位置
- ✅ 编辑位置
- ✅ 删除位置
- ✅ 查看列表（卡片形式）
...
### 5. Devices (设备管理) ✅
**文件**: `frontend/src/pages/Devices.tsx`
- ✅ 创建设备
- ✅ 编辑设备
- ✅ 删除设备
- ✅ 查看列表（卡片形式）
...
1. **创建按钮** - 页面右上角 "添加XXX" 按钮
2. **编辑按钮** - 每条记录的蓝色编辑图标 ✏️
3. **删除按钮** - 每条记录的红色删除图标 🗑️
4. **统计卡片** - 显示总数和分类统计
...
| 操作 | HTTP方法 | URL模式 | 说明 |
|------|----------|---------|------|
| 获取列表 | GET | `/api/v1/{resource}?tenant_id={id}` | 查询所有数据 |
| 创建 | POST | `/api/v1/{resource}` | 创建新记录 |
...
### 3. 访问应用
- 前端: `http://192.168.2.6:3000` 或 `http://localhost:3000`
...
2. **React Query** - 自动缓存和状态管理
3. **乐观更新** - 操作后自动刷新
4. **类型安全** - 完整TypeScript支持
5. **响应式设计** - TailwindCSS适配各种屏幕
6. **用户友好** - 删除操作有确认提示
...
1. ✅ 所有操作都会自动刷新列表（使用React Query的`invalidateQueries`）
2. ✅ 删除操作有确认提示（`window.confirm`）
3. ✅ Modal关闭时会清除选中状态
4. ✅ 支持localhost和局域网IP访问
5. ⚠️ 某些页面可能有TypeScript类型警告（不影响功能）
6. ⚠️ 确保后端服务已启动，否则页面显示空白
