...
| 1e | 触发异常检测 | 实时检测跌倒、生命体征异常等情况 | `backend/app/services/tdp_processor.py:203` |

---

## 流程2：多级告警触发与路由流程

**描述**：后端告警引擎 - 从异常检测到多通道通知发送，包含卡片路由逻辑
...
| 2b | 获取设备关联卡片 | 查找活跃床位或位置卡片以确定告警接收者 | `backend/app/services/alert_engine.py:89` |
| 2c | 解析告警接收者 | 基于卡片类型和用户标签确定通知目标（护士/管理员） | `backend/app/services/card_manager.py:134` |
...
**描述**：后端护理质量服务 - 从IoT数据分析到生成团队质量报告和评分
...
| 位置ID | 标题 | 说明 | 文件路径:行号 |
|--------|------|------|---------------|
| 3a | API触发报告生成 | 前端请求指定时间范围（7天/30天）的护理质量报告 | `backend/app/api/v1/care_quality.py:34` |
| 3b | 查询IoT时序数据 | 获取指定时间段内所有护工和住户的位置/活动数据 | `backend/app/services/care_quality.py:123` |
| 3c | 计算空间覆盖率 | 分析护工-住户距离，统计有效护理区域（1.2米内）时长 | `backend/app/services/care_quality.py:234` |
| 3d | 获取住户健康基线 | 14天观察期建立的个性化基线（睡眠/活动/生命体征） | `backend/app/services/baseline.py:456` |
...
| 位置ID | 标题 | 说明 | 文件路径:行号 |
|--------|------|------|---------------|
| 4a | 页面初始化 | 组件挂载时触发数据获取 | `frontend/src/pages/Dashboard.tsx:28` |
| 4b | 获取统计数据 | 调用后端API获取住户数、设备数、告警数等概览信息 | `frontend/src/pages/Dashboard.tsx:45` |
...
| 4e | 渲染生命体征图表 | 使用Recharts展示心率/呼吸率趋势（最近24小时） | `frontend/src/components/PermissionGuard.tsx:34` |

---

## 流程5：前端住户管理CRUD流程

**描述**：前端React页面 - 创建新住户的完整表单提交和数据同步流程
...
| 5d | 自动生成活跃床位卡片 | 为新住户创建床位监控卡片，配置告警路由 | `backend/app/services/card_manager.py:89` |
| 5e | 更新前端状态 | 将新创建的住户添加到列表，触发UI重新渲染 | `frontend/src/pages/Residents.tsx:203` |

---

## 流程6：JSON存储引擎核心操作

**描述**：后端存储服务 - 通用CRUD操作、序列化和缓存机制的实现
...
| 6b | 模型序列化 | 处理datetime/UUID/bytes/BaseModel等特殊类型转JSON | `backend/app/services/storage.py:67` |
...
| 6e | Lambda表达式查询 | 支持复杂过滤条件的高级查询（如时间范围、多字段匹配） | `backend/app/services/storage.py:234` |
...
│       │   │   ├── 心率阈值判断 [7c]
│       │   │   │   ├── L1级别: <45 或 >115 bpm
│       │   │   │   ├── L2级别: 45-54 或 96-115 bpm
│       │   │   │   └── L3级别: 55-89 或 90-95 bpm
│       │   │   └── 呼吸率阈值判断
│       │   │       ├── L1级别: <8 或 >26 rpm
│       │   │       └── L2/L3级别: 其他异常范围
│       │   └── 返回标准编码 [7d]
...
| 位置ID | 标题 | 说明 | 文件路径:行号 |
|--------|------|------|---------------|
| 7a | 触发姿态编码映射 | TDP处理器调用SNOMED服务转换原始姿态值 | `backend/app/services/tdp_processor.py:178` |
| 7b | 生命体征评估算法 | 根据HR/RR数值判断正常/L3/L2/L1级别 | `backend/app/services/snomed_service.py:89` |
| 7c | 心率阈值判断 | L2级别：45-54或96-115 bpm，L1级别：<45或>115 bpm | `backend/app/services/snomed_service.py:134` |
...
| 流程编号 | 流程名称 | 类型 | 关键技术 |
|---------|---------|------|---------|
| 1 | IoT设备数据接收与处理 | 后端数据处理 | TDP协议、Protobuf、异步IO |
| 2 | 多级告警触发与路由 | 后端告警系统 | 告警引擎、卡片路由、多通道通知 |
...
| 6 | JSON存储引擎 | 后端存储层 | 异步IO、LRU缓存、泛型CRUD |
| 7 | SNOMED CT医疗编码 | 后端标准化 | 医疗编码、HL7 FHIR、阈值判断 |
...
- **维护说明**: 此文档基于Codemap自动分析结果翻译，如代码结构变更请同步更新
