...
```
127.0.0.1:13468 显示
错误码: Request failed with status code 500
```
...
const createMutation = useMutation({
  mutationFn: async (data: any) => {
    const payload = {
      ...data,
      tenant_id: TENANT_ID,
      is_institutional: true,      // ✅ 新增：默认机构模式
      anonymous_name: data.last_name,  // ✅ 新增：匿名代称
      can_view_status: true,       // ✅ 新增：默认允许查看
    }
...
- **admission_date** - 入住日期
- **status** - 状态 (active/discharged/transferred)
- **is_institutional** - 是否机构模式 (默认true)
- **can_view_status** - 是否允许家属查看 (默认true)

### 可选字段
- **HIS_resident_id** - HIS系统ID
