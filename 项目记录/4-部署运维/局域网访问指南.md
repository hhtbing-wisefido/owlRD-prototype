...
# 首次运行：下载Swagger UI静态文�?python scripts/download_swagger_ui.py

# 启动服务（推�?�?- 自动检查端口冲突）
python start_with_check.py

# 说明：start_with_check.py 已自动配�?host=0.0.0.0
# 支持本机和局域网访问
```

**访问地址**:
- 本机: http://localhost:8000 �?http://192.168.2.6:8000
...
**访问地址**:
- 本机: http://localhost:3000 �?http://192.168.2.6:3000
...
1. 系统偏好设置 �?安全性与隐私 �?防火�?2. 点击 "防火墙选项"
3. 添加 Node.js �?Python 应用程序
4. 设置�?"允许传入连接"
...
**症状**: 访问 `http://192.168.2.6:8000` 超时或拒绝连�?
**解决方案**:
...
2. **检查防火墙**
   - Windows: 确认端口 8000 已添加到防火墙规�?   - Linux: `sudo ufw status`
   - 临时禁用防火墙测�? `sudo ufw disable`
...
2. **重启前端服务�?*（修�?`.env` 后必须重启）

3. **浏览器控制台检查错�?*
   - F12 打开开发者工�?   - 查看 Console �?Network 标签
   - 检查是否有 CORS 错误
...
1. **检查后端WebSocket端点**
   ```bash
   # 浏览器访�?   http://192.168.2.6:8000/api/v1/realtime/ws/10000000-0000-0000-0000-000000000001
   ```

2. **检�?`.env` 中的 WebSocket URL**
   ```bash
   VITE_WS_URL=ws://192.168.2.6:8000
   ```
...
2. Safari浏览器访�? `http://192.168.2.6:3000`
3. 可添加到主屏幕以获得类似App的体�?
### Android设备

1. 确保设备连接到同一WiFi
2. Chrome浏览器访�? `http://192.168.2.6:3000`
3. 可通过菜单 "添加到主屏幕"
...
2. **减少WebSocket重连间隔**
   ...
   ```typescript
   reconnectInterval = 1000  // �?000改为1000ms
   ```
...
```bash
# 启动（局域网访问�?uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload

# 健康检�?curl http://192.168.2.6:8000/health
