...
#### 2. 25_Alarm_Notification_Flow.md（业务流程层）
**告警级别说明**：
- **L1** (EMERGENCY): 紧急，高风险高置信，立即报警
- **L2** (ALERT): 警报，高危事件，网页+App通知
- **L3** (CRITICAL): 严重，网页显示
- **L5** (WARNING): 警告，分级确认
- **L8** (DEBUG): 调试，仅记录
- **L9** (CANCEL): 取消警报
...
#### 3. SQL表定义
- **13_iot_monitor_alerts.sql**: IoT设备报警配置表
- **14_cloud_alert_policies.sql**: 云端告警策略表
- ⚠️ **Alert实例记录表不在源SQL中** → 协议扩展实现
...
**15. 项目记录/2-源参考对照/1-数据库Schema对照/检查清单.md**
```markdown
...
```python
# 修正前
tenant = tenant_storage.get(tenant_id)  # ❌ StorageService没有get方法
...
```python
# 修正前
cards = await self.card_storage.find_all(filter_func)  # ❌ find_all不是async
...
```python
# 修正前
"email": "testuser@example.com"  # ❌ 重复冲突
...
# Device
new_device = {
    "comm_mode": "WiFi",  # ✅ 大小写敏感
    "status": "online",  # ✅ 不是installed
...
主要内容：
- 修正validation.py: alert_level + timestamp
- 修正init_sample_data.py: alerts创建数据
- 更新所有相关文件的源参考对齐注释（15个文件）
- 创建对齐分析报告文档
```

### Commit 2: 完成所有测试修复，达成100%通过率
```
Commit: 6bf435d
...
主要内容：
- Location字段修正（Institutional + door_number + timezone）
- IoT数据添加id字段
- 修复API端点（tenants, alert-policies, cards）
- 测试脚本完善（tenant_id + 所有创建数据字段）
- 生成10个测试历史报告
```
...
**发现**：Alert是协议扩展实现
- 源SQL中只有配置表（13_iot_monitor_alerts.sql, 14_cloud_alert_policies.sql）
- 没有alert实例记录表定义
- Alert模型完全基于TDPv2协议和25_Alarm_Notification_Flow.md

**启示**：
- 协议文档优先级 > SQL表结构 > Model实现
- 扩展实现必须严格对齐协议定义
- 文档中必须明确标注"协议扩展"
...
### Git提交
- [x] Commit 1: 37a62ee（Alert对齐）
- [x] Commit 2: 6bf435d（100%通过）
- [x] 修改统计：17个文件，2365行新增
