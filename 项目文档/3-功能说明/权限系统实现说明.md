...
�?  ├── auth.py                   # JWT认证依赖
�?  └── middleware/permissions.py # 权限检查辅助函�?├── 前端权限系统
�?  ├── hooks/usePermissions.ts   # 权限Hook
�?  ├── components/PermissionGuard.tsx # 权限守卫组件
�?  └── services/api.ts          # 自动添加Authorization�?└── 权限控制集成
    ├── 所有后端API端点         # 完整权限检�?    ├── 所有前端页面组�?        # UI权限控制
    └── 权限配置界面            # 动态权限管�?```
...
### 数据访问范围 (alert_scope)
- **ALL**: 查看租户下所有数�?- **LOCATION**: 查看匹配location_tag的数�?- **ASSIGNED_ONLY**: 查看分配给自己的住户数据
...
### 3. 权限中间�?(`middleware/permissions.py`)
```python
...
- `alerts.py` - 告警管理 (认证 + 租户检�?
- `alert_policies.py` - 告警策略 (Admin/Director)

#### 权限检查模�?1. **基础认证**: 所有端点都需要有效JWT token
...
export function usePermissions(): PermissionResult {
  // 计算所有权限标�?  const isAdmin = user?.role === 'Admin'
...
### 3. API服务集成 (`api.ts`)
```typescript
// 自动添加Authorization�?api.interceptors.request.use((config) => {
...
#### 🔄 部分完成的页�?- `Locations.tsx` - 位置管理 (导入已添加，按钮待包�?
- `Residents.tsx` - 住户管理 (需要完�?
- `Alerts.tsx` - 告警管理 (需要添�?
- `AlertPolicies.tsx` - 告警策略 (需要添�?
- `Dashboard.tsx` - 仪表�?(需要根据权限显�?

#### 权限控制模式
```typescript
// 模式1: 简单属性检�?<PermissionGuard requires="canManageUsers">
  <CreateButton />
</PermissionGuard>
...
### 功能特�?- **用户权限配置**: 管理员可配置用户的`alert_scope`和权限标�?- **实时权限预览**: 显示权限配置的实际效�?- **权限说明文档**: 详细解释各权限级别的含义
- **安全访问控制**: 仅Admin和Director可访�?
### 界面结构
```
权限配置页面
├── 用户列表 (左侧面板)
...
2. **租户隔离**: 严格的多租户数据隔离
3. **角色权限**: 基于角色的访问控�?4. **资源权限**: 细粒度的资源访问控制
5. **权限审计**: 记录所有权限相关操�?
### 前端安全
1. **客户端权限检�?*: 防止UI误操�?2. **Token自动管理**: 自动添加和刷新认证头
3. **权限状态同�?*: 实时反映权限变化
4. **降级显示**: 无权限时的友好提�?
## 部署和维�?
### 环境变量
...
- [ ] 数据库查询优�?- [ ] 前端权限状态管理优�?
### 功能扩展
- [ ] 更细粒度的资源权�?- [ ] 临时权限和权限委�?- [ ] 权限审批工作�?- [ ] 多租户权限继�?
### 安全加固
- [ ] 权限提升检�?- [ ] 异常访问模式分析
- [ ] 权限配置安全策略
- [ ] 定期权限审核
