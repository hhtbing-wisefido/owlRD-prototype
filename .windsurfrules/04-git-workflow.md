...
❌ 大杂烩:
commit 1: 添加用户注册、修复登录bug、更新文档、重构代码
```
...
**常见scope**:
- 模块名: `auth`, `user`, `order`, `product`
- 层级名: `api`, `ui`, `db`, `service`
- 文件名: `README`, `package.json`
- 功能名: `login`, `register`, `payment`
...
❌ 错误:
feat: 添加了用户认证功能        # "添加了"改为"添加"
Fix: 修复登录页面错误           # 不要大写首字母
docs: 更新部署指南。            # 不要句号
...
BREAKING CHANGE: API响应格式从 {data} 改为 {success, data, message}
...
BREAKING CHANGE: 
所有API响应格式从 {data} 改为 {success, data, message}
前端需要相应调整
...
# 或者添加新的commit来纠正
git commit -m "docs: 修正上个commit的说明"
```
...
- 📖 [GitHub Flow](https://guides.github.com/introduction/flow/) - 简化的工作流
