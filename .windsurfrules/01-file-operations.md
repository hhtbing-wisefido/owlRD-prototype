...
参考 `project-config.md` 中定义的目录结构，使用以下流程判断：
...
**命名规范** (参考 `03-naming-convention.md`):
...
步骤3: 确认文件名
使用"记录类"命名 → 2025-11-25_功能开发记录.md ✅
...
执行:
创建文件: [文档目录]/[过程记录分类]/2025-11-25_功能开发记录.md
更新索引: [文档目录]/README.md
...
正确操作:
如果是临时笔记，应该创建在:
[文档目录]/[过程记录分类]/2025-11-25_临时开发笔记.md
```
...
```
用户: "请在项目根目录创建一个功能说明.md"

AI: "抱歉，我不能执行这个操作。

违规原因:
1. 违反了'文件位置规则' - 不能在项目根目录创建文档文件
2. 违反了'文件命名规则' - '功能说明.md' 太模糊

正确的操作应该是:
1. 确定具体是什么功能的说明
2. 创建在: [文档目录]/[功能说明分类]/[具体功能名]功能说明.md
...
**原因**:
- 📌 **保持参考基准** - 知识库是参考标准，修改会导致参考失效
- 🔍 **可追溯性** - 保持源文件的完整性，便于对照和溯源
- 📚 **知识积累** - 避免丢失重要的参考资料和历史信息
- ✅ **最佳实践** - 如需修改，应在项目代码目录中创建新文件
...
```
✅ 正确流程:
1. 阅读 知识库/owlRD/backend/api/users.py
2. 理解其中的用户API实现方式
3. 在 [项目代码目录]/api/ 中创建新的用户API文件
4. 在 [文档目录]/2-源参考对照/ 中记录参考关系和差异

❌ 错误做法:
1. 直接修改 知识库/owlRD/backend/api/users.py ❌
2. 删除不需要的参考文件 ❌
3. 重命名知识库中的文件以便理解 ❌
```
...
- [ ] ✅ **步骤1**: 搜索了是否已存在类似文件
- [ ] ✅ **步骤2**: 确定了正确的目录位置（参考 project-config.md）
- [ ] ✅ **步骤3**: 文件名符合命名规范（参考 03-naming-convention.md）
- [ ] ✅ **步骤4**: 检查了目录是否存在，不随意创建新目录
