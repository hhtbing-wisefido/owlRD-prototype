...
- 示例：
  - `project-code/`  可能是 `src/`, `project-code/`, `app/` 等
...
**强制要求**：
- ✅ 每个项目**必须**有且只有一个代码主目录
- ✅ 代码主目录**必须**命名为 `project-code`
- ❌ 禁止使用其他名称（如 src/, app/, code/ 等）
...
#### 场景1：检测到多个代码目录
`
如果存在多个代码目录（如 backend/, frontend/, src/ 等在根目录）：
1. 创建 project-code/ 目录
2. 将所有代码目录移动到 project-code/ 下
3. 更新所有引用路径
...
#### 场景2：检测到一个代码目录
`
如果存在一个代码目录（如 src/, app/, code/ 等）：
1. 将该目录重命名为 project-code/
2. 更新所有引用路径
3. 更新 README.md 和相关文档
4. 提交到 Git
`
...
**以下目录被识别为代码目录**：
- 包含 `.py`, `.ts`, `.tsx`, `.js`, `.jsx`, `.java`, `.go`, `.rs` 等代码文件
- 包含 `package.json`, `requirements.txt`, `Cargo.toml`, `go.mod` 等依赖文件
- 目录名为常见代码目录名：`src`, `app`, `code`, `backend`, `frontend`, `server`, `client`
...
function Complete-Standardization {
    ...
    if ($valid) {
        ...
        git add -A
        git commit -m "refactor: 项目标准化`n`n- 重命名 $OldCodeDir  project-code/`n- 重组 $OldDocsDir  [文档目录]/`n- 按标准分类所有文档`n- 更新所有路径引用`n- 更新README和目录树"
